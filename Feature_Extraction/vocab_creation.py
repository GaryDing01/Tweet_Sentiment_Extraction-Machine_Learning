...
def read_lines(content_filepath, content_type=None):
    ...
    strip_lines = [re.sub('[^A-Za-z]+', ' ', line).strip().lower() for line in lines]


    # 去掉stopwords
    stopwords_lines=remove_stopwords(strip_lines)

    # print('lines:')
    # print(stopwords_lines)
    # exit()
...
def remove_stopwords(lines):
    '''
    在原词中去除stopwords
    :param lines: 原来构建的句子列表
    :return: 新构建的lines
    '''
    StopWords=stopwords.words("english")
    for i in range(len(lines)):
        lines[i]="".join(word+" " for word in lines[i].split() if word not in StopWords)
    return lines
...
def load_corpus_content(max_token=-1):
    ...
    lines = read_lines('../Data/train_all.csv')  # 序号314无推特文字信息
